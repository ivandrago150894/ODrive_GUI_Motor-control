...
2. Depending on your choice, the GUI will display different things.

==> None of the goals were achieved. Hoever, now position control is posible and the GUI can be displayed
    without the need to connect the ODrive Previously.
    Bsesides, A connect button was added, once is clicked, the ODrive connection procces is initiated.

The following modifications has been made to the code:

1. A connect button was added
2. The 
...
class MainWindow(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        global odrv, axis, motor, motor_config, ctrl_config, gear_ratio,\
            t_buffer, tau_d_buffer, tau_buffer, gearbox_buffer, current_buffer, voltage_buffer, pos_buffer, q_d_buffer,\
            t_0, t_minus_1,\
            tau_d, q_d,\
            ConnectButton, LoggingButton, TRefBox,\
            Connection_Flag

        self.setWindowTitle('ODrive + T-Motor GUI')
        ...
        ConnectButton = QPushButton("Connect ODrive")
...
def Connect_Clicked():
    global odrv, axis, motor, motor_config, ctrl_config, gear_ratio, Connection_Flag, t_0
